...
public class KeyProvider {
	...
	public static PrivateKey getPrivateKey(String password) throws IOException, InvalidKeyException {
		File keyFile = new File(AppUtils.getContext().getFilesDir(),
				"private.key");
		...
		try {
			privateKey = KeyFactory.getInstance(ASYMMETRIC_ALG).generatePrivate(
					privateKeySpec);
	...
	public static PublicKey getPublicKey() {
		...
		try {
			...
			PublicKey publicKey;
			try {
				publicKey = KeyFactory.getInstance(ASYMMETRIC_ALG).generatePublic(
						keySpec);
			} catch (InvalidKeySpecException e) {
				throw new Error(e);
			} catch (NoSuchAlgorithmException e) {
	...
	private static void saveKeys(KeyPair keyPair, String password) {
		...
		try {
			...
			FileOutputStream fos = AppUtils.getContext().openFileOutput(
					"public.key", 0);
			...
			byte[] cryptedPrivate = encryptKey(
					pkcs8EncodedKeySpec.getEncoded(), password);
	 ...
	 */
	public static byte[] symmetricEncrypt(SecretKey encryptionKey,
	...
	public static byte[] symmetricDecrypt(SecretKey key,
			FileInputStream cryptedStream) throws InvalidKeyException {
		try {
			Cipher cipher = Cipher.getInstance(KeyProvider.SYMMETRIC_ALG);
			cipher.init(Cipher.DECRYPT_MODE, key);
			ByteArrayOutputStream baos = new ByteArrayOutputStream(
					cryptedStream.available());
	...
	public static SecretKey recoverSecretKey(PrivateKey privateKey,
			byte[] cryptedKey) throws IOException, InvalidKeyException {
		try {
			Cipher cipher = Cipher.getInstance(ASYMMETRIC_ALG);
	...
	public static SecretKey getRandomKey() {
		...
		try {
			...
			return new SecretKeySpec(kgen.generateKey().getEncoded(),
					SYMMETRIC_ALG);
		} catch (NoSuchAlgorithmException e) {
			throw new Error(e);
		}
